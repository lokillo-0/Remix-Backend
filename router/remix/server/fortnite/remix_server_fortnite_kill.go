...
func POSTVbucksPerKill(c *gin.Context) {
	if utilities.GetConfig().VBUCKS_PER_KILL <= 0 {
	...
	vbucks["quantity"] = current + utilities.GetConfig().VBUCKS_PER_KILL
	...
	c.JSON(200, gin.H{"granted": utilities.GetConfig().VBUCKS_PER_KILL})
}
